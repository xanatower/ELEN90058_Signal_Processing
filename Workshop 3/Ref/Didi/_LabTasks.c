...
float process_time(float x0)
{
    ...
    static float x_input[BUFFER_SIZE] = {0.0};          // BUFFER_SIZE = (M-1) is defined in 'Params.h'
    static int current = 0;


    float y = b[0] * x0;
    for (int i = 1; i <= BUFFER_SIZE; i++) {
        y += b[i] * x_input[REM(current-i)];
    }
    ...
    /*
    float y = b[0] * (x0 + x_input[REM(current)]);      // Macro 'REM(current)' is defined in 'Params.h'
    // h[0]x[n] + h[M - 1]x[n - M + 1]

    x_input[current] = x0;
    // save current x0 into x_input after 'y' is calculated, thus the size of 'x_input' can be reduced by 1 (from M to M-1).

    for (int i = 1; i <= BUFFER_SIZE/2-1; i++) {
        y += b[i] * (x_input[REM(current-i)] + x_input[REM(current+i)]);
    }
    // (h[1]x[n-1] + h[M - 2]x[n - M + 2]) + ... + (h[(M-1)/2-1]x[(M-1)/2+1] + h[(M-1)/2+1]x[(M-1)/2-1])

    y += b[BUFFER_SIZE/2] * x_input[REM(current-BUFFER_SIZE/2)];
    // h[(M-1)/2]x[(M-1)/2]
	*/
    current++;
    current %= BUFFER_SIZE;
